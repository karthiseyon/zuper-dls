...
The styleguide ships ~190 named glyphs, including a core UI set (home, search, check, x, plus, arrows, chevrons, copy, download, upload, link, refresh, filter, list, grid, settings, sliders, edit, trash, eye, lock, bell, info, warning, help, star, heart, tag, pin, clock, calendar, play/pause, doc, folder, clipboard, image, camera, chart/chart‑line/chart‑pie, mail, chat, send, phone, megaphone, share, users, user, card, invoice, receipt, dollar, gauge, target, package, map, cpu, robot, sparkle, zap, shield, qr, barcode …) **plus field‑service & industry icons** for Zuper's trades: thermometer, snowflake, fan, wind (HVAC); pipe‑wrench, droplet (plumbing); plug, bolt (electrical); solar‑panel, sun (solar); house, crane, hard‑hat, ruler (roofing/construction); bug (pest control); tree, plant, leaf, flower (landscaping); security‑camera, siren, lock (security); broom, spray (cleaning); washing‑machine (appliance); broadcast, cell‑signal (telecom); fire‑extinguisher, first‑aid (safety); plus FSM workflow icons (truck, van, wrench, toolbox, hammer, screwdriver, warehouse, storefront, factory, gas‑pump, handshake, signature, user‑gear, users‑three, address‑book, calendar‑plus, clock‑countdown, checklist, check‑square, route/path, navigation, bank, money, presentation). The **Iconography** page has an **Icon Studio**: search Zuper's set *or the full ~1,500‑glyph Phosphor library* by name or tag (e.g. "home"→house, "roof"→house, "payment"→credit‑card), then recolour, resize, set a background and export SVG / PNG / `icon("name")`. Need a glyph that isn't there? Pull the matching Phosphor `regular` weight — it's automatically on‑brand.

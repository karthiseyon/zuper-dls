...
7. **Iconography** — Zuper's one icon language: a **single‑weight outline set** (clean outline glyphs, 256px grid, `currentColor`). The consistency rules, plus an **Icon Studio** — search Zuper's ~190 set *or the full ~1,500‑glyph Phosphor library* (trade synonyms like "roof"→house), recolour / resize / set a background, then copy SVG, copy `icon("name")`, or download SVG/PNG. Everything comes out on‑brand.
